...
function setup() {
    ...
    repo.loadManifest([
        ...
        {id:'clear_b',src:"images/comment/clear_buttom.png"},
        {id:'clear_b_s',src:"images/comment/clear_buttom(shadow).png"},
        {id:'send',src:"images/comment/send_buttom.png"},
        {id:'send_s',src:"images/comment/send_buttom(shadow).png"},
    ]);
    for(let i=5;i<=22;i++)
    {
        let j=i-4
    ...
    for(let i=1;i<=15;i++)
    {
        if(i!=8)
        {
            repo.loadManifest([{id:'toilet_'+i,src:"images/toilet/toliet/toilet_"+i+".png"}]);
        }
    }
...
function draw() {
    if (scene == 0) {
        ...
        start_b.set({x: 600 - start_b.image.width * scale / 2, y: 3350 * 0.24, scaleX: scale, scaleY: scale});
        start_b_s.set({x: 600 - start_b.image.width * scale / 2, y: 3350 * 0.24, scaleX: scale, scaleY: scale});
        ...
        ok.set({x: 600 - ok.image.width * scale / 2, y: 3350 * 0.24, scaleX: scale, scaleY: scale});
        ok_s.set({x: 600 - ok.image.width * scale / 2, y: 3350 *0.24, scaleX: scale, scaleY: scale});
        ...
        stage.addEventListener("stagemousemove", function () {
            for (let i in map) {
                map_display(map[i], i)
                if(map_select(stage.mouseX,stage.mouseY,map[i].x,map[i].y,map[i].image.width*scale,map[i].image.height*scale))
                {
                    if(i!=0) {
                        select = i;
                    }
        ...
        if (question_n == 0) {
            ...
            order.push(12)
            startd =new Date()
            starts =startd.getTime()
            console.log(starts)
        }
        next(4)
    }
    else if (scene>=4&&scene<=13) {
        question_display(order[scene-4],scene-4)

        stage.on('click', function () {
            if((choose==0||choose==1)&&scene>=4&&scene<=13) {
                score_total = score_total.concat(score[scene-4][choose])
                next(scene + 1)
            }
        })
        ...
        endd =new Date()
        ends =endd.getTime()
        if(ends-starts<15000)
        {
            score_total=[]
        }
        console.log(location,ends-starts)
        score_total = score_total.concat(location_score[location-1])
    ...
    else if(scene==15)
    {
        let again = new createjs.Bitmap(repo.getResult('again'))
        let again_s = new createjs.Bitmap(repo.getResult('again_s'))
        let send= new createjs.Bitmap(repo.getResult('send'))
        let send_s = new createjs.Bitmap(repo.getResult('send_s'))
        let comment_bg = new createjs.Bitmap(repo.getResult('comment_bg'))
        comment_bg.set({x: 0, y: 0, scaleX: scale, scaleY: scale});
        again.set({x: 640, y: 640, scaleX: scale, scaleY: scale});
        again_s.set({x: 640, y: 640, scaleX: scale, scaleY: scale});
        send.set({x: 830, y: 640, scaleX: scale, scaleY: scale});
        send_s.set({x: 830, y: 640, scaleX: scale, scaleY: scale});
        stage.addChild(comment_bg);
        stage.addChild(again);
        stage.addChild(send);
        var commentText =new TextInput();

        commentText.placeHolder = "Input Field";
        commentText.set({x:205,y:422})//,scaleX :,scaleY:5});
        stage.addChild(commentText);
        stage.addEventListener("stagemousemove", function () {
            if (stage.mouseX >= again.x && stage.mouseX <= again.x + again.image.width * scale && stage.mouseY >= again.y && stage.mouseY <= again.y + again.image.height * scale && scene == 15) {
            ...
            if (stage.mouseX >= send.x && stage.mouseX <= send.x + send.image.width * scale && stage.mouseY >= send.y && stage.mouseY <= send.y + send.image.height * scale && scene == 15) {

                stage.removeChild(send);
                stage.addChild(send_s);
            }
            else {
                if (scene == 15) {
                    stage.removeChild(send_s);
                    stage.addChild(send);
                }
            }
        })
